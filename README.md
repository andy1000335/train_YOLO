...
3. 訓練  
  (1) 使用 weights 檔訓練  
  $nbsp```python ./flow --load bin/yolov2.weights --model cfg/yolov2_test.cfg --train --annotation train/annotation --dataset train/image```  
  (2) 不使用 weights 檔訓練  
  $nbsp```python ./flow --model cfg/yolov2_test.cfg --train --annotation train/annotation --dataset train/image```  
